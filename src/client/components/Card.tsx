...
interface CardProps {
    ...
    setPnls: React.Dispatch<React.SetStateAction<number[]>>;
}
...
const Card: React.FC<CardProps> = ({investment, setPnls}) => {
    ...
    const stockPnl = stock
    ? ((Number(stock["05. price"]) * numberOfShares) - (priceBought * numberOfShares)).toFixed(2)
    : "0.00";
    const pnlPercentageChange = stock
    ? (((Number(stock["05. price"]) - priceBought) / priceBought) * 100).toFixed(2)
    : "0.00";
